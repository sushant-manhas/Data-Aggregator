...
start_date = st.date_input("Select the Start Date", value="2025-01-01")
end_date = st.date_input("Select the End Date", value="2025-01-01")
...
def fetch_and_process_data():
    ...
    def fetch_market_data():
        return client.search.markets.retrieve(sort_by='pricefeed_market', limit=98)

    sales_pricefeed_markets = fetch_market_data()
    sales_pricefeed_markets_City = sales_pricefeed_markets[sales_pricefeed_markets['location_type'] == 'CITY']
    sales_pricefeed_markets_Country = sales_pricefeed_markets[sales_pricefeed_markets['location_type'] == 'COUNTRY']
    sales_pricefeed_markets = pd.concat([sales_pricefeed_markets_City, sales_pricefeed_markets_Country], ignore_index=True)
